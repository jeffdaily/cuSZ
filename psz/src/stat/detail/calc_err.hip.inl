    ...
    T* d_odata, T odata_avg, T* d_xdata, T xdata_avg, size_t len, T h_err[4])
{
  ...
  hipMalloc(&d_sum_corr, sizeof(T));
  hipMalloc(&d_sum_err_sq, sizeof(T));
  hipMalloc(&d_sum_var_odata, sizeof(T));
  hipMalloc(&d_sum_var_xdata, sizeof(T));

  hipMemset(d_sum_corr, 0, sizeof(T));
  hipMemset(d_sum_err_sq, 0, sizeof(T));
  hipMemset(d_sum_var_odata, 0, sizeof(T));
  hipMemset(d_sum_var_xdata, 0, sizeof(T));
  ...
  hipMemcpy(&h_err[SUM_CORR], d_sum_corr, sizeof(T), hipMemcpyDeviceToHost);
  hipMemcpy(&h_err[SUM_ERR_SQ], d_sum_err_sq, sizeof(T), hipMemcpyDeviceToHost);
  hipMemcpy(&h_err[SUM_VAR_ODATA], d_sum_var_odata, sizeof(T), hipMemcpyDeviceToHost);
  hipMemcpy(&h_err[SUM_VAR_XDATA], d_sum_var_xdata, sizeof(T), hipMemcpyDeviceToHost);

  hipFree(d_sum_corr);
  hipFree(d_sum_err_sq);
  hipFree(d_sum_var_odata);
  hipFree(d_sum_var_xdata);
