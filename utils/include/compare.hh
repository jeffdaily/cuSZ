...
std::tuple<T, T, T, T> GPU_probe_extrema(T* in, size_t len)
{
  ...
  if constexpr (R == CUDA or R == HIP)
    ...
    static_assert(R == CUDA or R == HIP or R == SYCL, "GPU_probe_extrema supports CUDA / HIP / SYCL.");
  return {r[0], r[1], r[2], r[3]};
}
...
void assess_quality(psz_stats* s, T* xdata, T* odata, size_t const len)
{
  if constexpr (P == SEQ)
    ...
    static_assert(P == SEQ or P == CUDA or P == HIP or P == SYCL, "assess_quality: unsupported backend.");
}
