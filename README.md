...
pSZ/cuSZ (cuSZ for short) is a GPU implementation of the seminal [SZ algorithm](https://github.com/szcompressor/SZ). It is the *first* GPU-practical framework of error-bounded lossy compression on GPU for scientific data (circa 2020), aiming to improve SZ's throughput on heterogeneous HPC systems. pSZ/cuSZ primarily focuses on CUDA backend support, with other GPU-parallel backends in development. In addition to the CUDA backend, pSZ/cuSZ also builds and runs on AMD GPUs through the ROCm/HIP backend (select it at configure time with `-DPSZ_BACKEND=HIP` and set `CMAKE_HIP_ARCHITECTURES` to your target, e.g. `gfx90a` or `gfx1100`). pSZ/cuSZ is formerly known as cuSZ, which is also the short form of its current name. 
