    ...
    hipStream_t stream)
{
  ...
  hipMalloc(compressed, comp_len);
  hipMemcpy(*compressed, p_compressed, comp_len, hipMemcpyDeviceToDevice);

  psz_release(compressor);
}

void demo_decompress(uint8_t* compressed, psz_header* header, hipStream_t stream)
{
