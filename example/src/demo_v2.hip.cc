...
void f4demo_compress_v2(
    ...
    size_t* compressed_len, hipStream_t stream)
{
  ...
  hipMallocManaged(compressed, *compressed_len);
  hipMemcpy(*compressed, d_internal_compressed, *compressed_len, hipMemcpyDeviceToDevice);
...
void f4demo_decompress_v2(psz_header* header, uint8_t* compressed, hipStream_t stream)
{
...
void f8demo_decompress_v2(psz_header* header, uint8_t* compressed, hipStream_t stream)
{
  auto m = psz_create_resource_manager_from_header(header, stream);
...
void f4demo(std::string fname, psz_len3 len3, psz_predictor predictor)
{
  ...
  // clean up
  hipFree(compressed);
  hipFree(f4d_uncomp), hipFreeHost(f4h_uncomp);
  hipFree(f4d_decomp), hipFreeHost(f4h_decomp);
...
void f8demo(std::string fname, psz_len3 len3, psz_predictor predictor)
{
  ...
  // clean up
  hipFree(compressed);
  hipFree(f8d_uncomp), hipFreeHost(f8h_uncomp);
  hipFree(f8d_decomp), hipFreeHost(f8h_decomp);
  hipStreamDestroy(stream);
}
